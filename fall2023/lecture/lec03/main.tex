...
\sigma(x)=\max(0,x).
\]
\item Much \textcolor{Green}{faster} to calculate the function value and 
its derivatives.
...
\[
    o^{(j)} = h^{(j)}({\color{blue}o^{(j-1)}})=\sigma\left(W^{(j)}{\color{blue}o^{(j-1)}}+b^{(j)}\right),\text{ for }j=2,\ldots,L
\]
...
\begin{frame}
    ...
    \end{figure}
        ...
        \begin{column}{0.4\columnwidth}
            ...
            \onslide<6->{\think{Which computation can be parallelized?}}
        \end{column}
        }
    \end{columns}
    \pdfnote{
    ...
    \begin{columns}
        ...
        \begin{column}{0.6\textwidth}
            ...
            \begin{itemize}[<+->]
                ...
                \item[] {\em Keys for values should be learned}
                \item A word is related to multiple words in a sentence
                \item[] {\em Query should be matched to multiple keys}
            \end{itemize}
        \end{column}
    \end{columns}
\end{frame}
