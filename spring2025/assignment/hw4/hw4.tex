...
\title{\textbf{CS-GA.2590: Natural Language Processing, Spring 2025} \\HW-4: Scaling Language Models and Prompt Engineering}
\author{Name \\
NYU ID}
\date{}
...
\rhead{\today}
\lfoot{}
\cfoot{CS-GA.2590: Natural Language Processing, Spring 2025}
\rfoot{\thepage}
...
\section*{Due Date - 11:59 PM 04/09/2025}
...
\end{enumerate}
Specifically, we will use API from \href[]{https://api.together.xyz/playground/chat/togethercomputer/llama-2-70b-chat}{together.ai} to teach the \href{https://arxiv.org/abs/2307.09288}{LLaMa-2} 13B model to add three positive 5-digit integers.
...
We provided you a notebook \texttt{addition\_prompting.ipynb}. In the first section, there are two examples of zero-shot addition: ten instances of three 1-digit addition and ten instances of three 5-digit additions.
\begin{enumerate}[label=\alph*.]
    \item (2 points, written) Run the two examples. In your opinion, what are some factors that cause language model performance to deteriorate from 1 digit to 5 digits?
    \newpage
    \item (5 points, written) Play around with the config parameters in together.ai's \href{https://api.together.xyz/playground/chat/meta-llama/Llama-2-13b-chat-hf}{web UI}.
    \begin{itemize}
        \item What does each parameter represent? The parameters to be probed are: Temperature, Max Tokens, Top P, Top K, and Repetition Penalty.
        \item How does increasing each parameter change the generation?
    \end{itemize}
    \newpage
    \item (2 points, written) Do 5-digit addition with 70B parameter llama model.
    \begin{itemize}
        \item How does the performance change?
        \item What are some factors that cause this change?
    \end{itemize}
    \newpage
    \item (2 points, written) Previously we gave our language model the prior that the sum of three 5-digit numbers must have a maximum of 6 digits (by setting \texttt{max\_token=6}). What if we remove this prior by increasing the \texttt{max\_token} to 20?
    \begin{itemize}
...
In this part We will try to improve the performance of 5-digit addition via in-context learning.
For cost-control purposes (you only have \$25 free credits), we will use \href{https://api.together.xyz/models/meta-llama/Llama-2-13b-chat-hfb}{llama-2-13b}.

\begin{enumerate}[label=\alph*.]

\item (1 points, written) Using the baseline prompt ("Question: What is 3+7? Answer: 10 Question: What is a+b? Answer:"), check 1-digit addition for 10 pairs again. 
\begin{itemize}
    \item Compared to zero-shot 1-digit additions with maximum 2 tokens, how does the performance change when we use the baseline in-context learning prompt? 
    \item What are some factors that cause this change?
\end{itemize}
\newpage
\item (2 points, written) Now using the same incontext example, let's see if we can improve the performance of 5-digit addition. Check 5-digit addition for 10 pairs again.
\begin{itemize}
    \item Compared to zero-shot 5-digit additions with maximum 6 tokens, how does the performance change when we use the baseline in-context learning prompt?
    \item What are some factors that cause this change?
\end{itemize}
\newpage
...
In this part, you will compete with your classmates to see who is best at teach llama to add three 5-digit numbers reliably! Note: You can utilize a txt file to test a specific prompt but make sure the final prompt is in the \textbf{submission.py}. 

The autograder will test your prompt on 30 instances of 5-digit integer addition. Some pairs are manually designed and some pairs are randomly generated using a random seed that is different from \texttt{addition\_prompting.ipynb}. This will generate 30 API calls, with 1 second wait time between each API call. Since prompting has randomness, we will run the same trial 3 times and report the average performance. In total, each submission will incur 90 API calls. \\
...
\begin{enumerate}[label=\alph*.]
    ...
    \begin{itemize}
        \item get average accuracy greater than 0.2, or
        \item get average mean absolute error less than 1e5.
    \end{itemize}
