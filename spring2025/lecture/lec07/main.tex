...
\title[CSCI-GA.2590]{Scaling language models}
\author[He He]{He He\\
}
\institute[NYU]{
...
\begin{frame}
    {Logistics}
    \begin{wideitemize}
        \item We will drop the lowest quiz.
        \item Solutions will be put up on campuswire.
        \item Plan for today:
            \begin{itemize}
                \item LM basics for 30min (He)
                \item Scaling laws (Nick)
            \end{itemize}
    \end{wideitemize}
\end{frame}
...
\begin{frame}
    ...
    Key idea:\\
    \begin{itemize}
        \item Pretraining on large textual data to learn many tasks/capabilities
        \item Finetuning to adapt the model to downstream tasks
    \end{itemize}

    Challenges in large scale pretraining:
    \begin{itemize}
        \item Tokenize multilingual and multimodal data
        \item Efficient training over long sequences 
        \item Gradient vanishing/exploding, numerical stability
    \end{itemize}
...
\begin{frame}
    ...
    \begin{itemize}[<+->]
        ...
        \item Being more likely could entail many things (e.g., grammaticality, commonsense)
    \end{itemize}
...
\begin{frame}
    ...
    {What if we don't have the stop symbol?}\\\pause
    \begin{itemize}
...
\begin{frame}
    ...
    $$
    \ell({D}) = \sum_{x\in\sD}\sum_{i=1}^{|x|} \log p_\theta(x_i\mid x_{1:i-1}) \;,
    $$
    \vspace{-2em}
 ...
 $$
    ...
    \begin{itemize}[<+->]
        ...
        \item Intuition: a model of perplexity $k$ effectively chooses from $k$ words (uniformly) when making predictions. 
        \item What is the minimum and maximum PPL you can get?
