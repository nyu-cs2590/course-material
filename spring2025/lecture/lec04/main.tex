...
\begin{frame}
    ...
    \begin{wideitemize}
        ...
        \item They are often called {\bf encoders}: take a sequence of tokens and output a sequence of embeddings
        ...
        \item Three building blocks for encoders:
            \begin{itemize}
        ...
        \begin{column}{0.6\textwidth}
            \begin{wideitemize}
                \item Multi-head self-attention
                    \begin{itemize}
                        \item Compute contextualized representations 
                    \end{itemize}
                \item Positional encoding 
                    \begin{itemize}
                        \item Represent the order of tokens
                    \end{itemize}
...
\begin{frame}
    ...
    \vspace{-2em}
    \begin{itemize}
        \item Each row is an embedding for a particular position
        \item Each column is a sinusoidal wave with a particular frequency
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{wideitemize}
        ...
        \item It happens to all \blue{deep} networks (which are \red{hard to optimize}).
            \pause
        \item In principle, a deep network can always represent a shallow network (by setting higher layers to \blue{identity functions}), thus it should be at least as good as the shallow network.
...
\begin{frame}
    ...
    \begin{itemize}
        \item Normalize each input sample to zero mean and unit variance \mycite[https://arxiv.org/pdf/1607.06450.pdf]{[Ba et al., 2016]}
    ...
    \item Robust to varying sequence length in a batch
    \end{itemize}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        \item Main reason: {\bf training stability} for {\it deep} neural networks (avoiding NaN, diverging loss, etc.)
        \item Sources of instability:
            \begin{itemize}
...
\begin{frame}
    ...
    \vspace{-2em}
    \begin{itemize}
        \item Add (residual connection) \& Normalize (layer normalization) for the output of self-attention and FFN (post-LN)
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{figure}
        \includegraphics[height=5cm]{figures/pre-post-LN}
        \caption{From \mycite[https://arxiv.org/pdf/2002.04745]{[Xiong et al. 2020]}}
    \end{figure}
    \begin{itemize}
        \item Post-LN: normalize the output of each layer
        \item Pre-LN: normalize the input of each layer
        \item Use either or both
    \end{itemize}
\end{frame}
...
\begin{frame}
    {Putting everything together}
    \begin{figure}
        \includegraphics[height=4cm]{figures/ffn}
    \end{figure}
    \vspace{-1em}
    \begin{itemize}
        \item Same FFN applied to each embedding 
        \item Two layers: first layer \blue{expands} the dimension (e.g., $d \to 4d$), second layer projects it back (e.g., $4d \to d$)
    \end{itemize}
\end{frame}
...
\begin{frame}
    {Sequence generation}
    \begin{wideitemize}[<+->]
        \item Given a sequence of contextualized embeddings, we can do classification.
...
\begin{frame}
    ...
    Decoder: Which \blue{input tokens} are most relevant for generating the \red{next output} token? 
...
\begin{frame}
    \frametitle{Contrastive Decoding}

    \begin{wideitemize}
        \item \textbf{Problem:} Greedy or beam search decoding can lead to repetitive or bland outputs.
        \item \textbf{Key Idea:} Such errors are more prominent in weaker/smaller models, so we can use a weaker model to penalize such errors \mycite[https://arxiv.org/pdf/2210.15097]{[Li et al., 2023]}
            \begin{figure}
                \includegraphics[height=5cm]{figures/cd}
            \end{figure}
    \end{wideitemize}
\end{frame}
...
\begin{frame}
    \frametitle{Contrastive Decoding}
    \begin{itemize}
        \item Vanilla CD score:
                \[
                    \underbrace{\log p_{\text{strong}}(y_t \mid y_{<t})}_{\text{standard likelihood}}
                    \; - \underbrace{\log p_{\text{weak}}(y_t \mid y_{<t})}_{\text{weak model penalty}},
                \]
        \item Remove (-inf score) implausible tokens $x_i\in\sV$ where
            \[
                p_{\text{strong}}(x_i \mid x_{<i}) < \alpha \max_{w\in\sV} p_{\text{strong}}(w \mid x_{<i})
            \]
            Avoid low probability tokens that happen to have large contrast score
        \item Run beam search using CD score
    \end{itemize}
\end{frame}
