...
\begin{frame}
    ...
    \begin{itemize}
        \itemsep1em
        \item Last lecture: aligning LMs with human preferences by prompting and supervised learning
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item We assume the pairwise ranking follows the Bradley-Terry-Luce model:
            $$
            p_\theta(y_1 \succ y_2 \mid x) = \frac{\exp(r_\theta(x,y_1))}
...
\begin{frame}
    {Set up}
    \begin{wideitemize}
        \item We have pairwise preference data $(x, y_1,  y_2)$ (assuming $y_1$ is preferred over $y_2$)
        \item Can we learn a policy $\pi_\theta$ that maximizes $p(y_1 \succ y_2)$?
        \item Recall: how do we model the probability?
            $$
            p(y_1 \succ y_2 \mid x) = \frac{1}{1+\exp(-(r(x,y_1) - r(x,y_2)))}
            $$
...
\begin{frame}
    ...
    \begin{wideitemize}[<+->]
        \item MLE objective on the preference dataset:
            $$
            \min - \mathbb{E}_{(x,y_1,y_2)\sim \sD}\log p_\theta(y_1 \succ y_2 \mid x) = - \mathbb{E}_{(x,y_1,y_2)\sim \sD} \pb{\frac{1}{1+\exp(-(r_\theta(x,y_1) - r_\theta(x,y_2)))} }
            $$
        ...
        \item we get the DPO objective ($\sigma$ is the logistic function)
            $$
            \min - \mathbb{E}_{(x,y_1,y_2)\sim \sD} \log \sigma \p{
                \beta\log\frac{\pi_\theta(y_1\mid x)}{\pi_0(y_1\mid x)} - 
                \beta\log\frac{\pi_\theta(y_2\mid x)}{\pi_0(y_2\mid x)}
            }
            $$
...
\nabla_\theta \mathcal{L}_{\text{DPO}}(\theta) =
        - \, \beta \, \mathbb{E}_{(x, y_1, y_2) \sim \mathcal{D}} \left[ \hat{p}_\theta(y_2\succ y_1) \left( \nabla_\theta \log \pi(y_1 \mid x) - \nabla_\theta \log \pi(y_2 \mid x) \right) \right],
    \end{align*}
    where
    \begin{align*}
\hat{p}_\theta(y_2\succ y_1) = \sigma \p{
                \beta\log\frac{\pi_\theta(y_2\mid x)}{\pi_0(y_2\mid x)} - 
                \beta\log\frac{\pi_\theta(y_1\mid x)}{\pi_0(y_1\mid x)}
            }
    \end{align*}
